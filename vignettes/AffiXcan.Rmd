...
functions support parallelization. The user has to indicate the number of cores
that can be used by AffiXcan to parallelize the tasks.
[BiocParallel package](https://bioconductor.org/packages/release/bioc/html/\
...
a regulatory regions, rather then only those SNPs with a greater effect size on
